...
graph TD
    ...
    PKG -. dev .-> VITE
    PKG -. build .-> VITE
    PKG -. deploy: "$npm_execpath run build && wrangler deploy" .-> EXEC
    EXEC -. run build .-> VITE
    EXEC -. then .-> WRG
    PKG -. cf-typegen .-> WRG
    PKG -. preview .-> VITE
    ...
    PW -- "webServer: npm run dev<br/>--port 5199 --strictPort" --> PKG
```

> Tip: if your renderer chokes on the `$npm_execpath` in the quoted edge label,
> just remove the quotes around that label (`deploy: $npm_execpath ...`) — it is
> the only node/label carrying `$`.
