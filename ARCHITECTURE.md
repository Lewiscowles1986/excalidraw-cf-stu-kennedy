...
- **It IS stored server-side.** Every attributed edit upserts it into the room-local `contributors` table (`do/drawing-room.ts`, user_id + last_seen_at) of the room edited. There is deliberately **no** per-user global registry DO — cross-room "rooms you've edited" queries instead fan out to the rooms the client names, via `POST /api/rooms/accessible`, and only rooms whose contributors include the userId are returned.
...
The SQLite `elements` table holds **drawing geometry/shape properties** — coordinates, colors, stroke widths, and any *text the user types into a shape*. On top of that, the attribution writes persist `userId` in the `contributors` table of each edited room's DrawingRoom (described above). The live `sessions` Map holding `userId`/`username` per open WebSocket is still **in-memory only** — but unlike presence, the attribution rows are durable by design: they must survive eviction for cross-room access listing to work.
